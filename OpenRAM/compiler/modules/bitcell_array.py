    ...
    def get_cell_name(self, inst_name, row, col):
        ...
        if (row, col) not in self.cell_inst:
            ...
            if OPTS.analytical_delay:
                ...
                # Find a valid cell to use as a template
                valid_row = min(row, self.row_size - 1)
                valid_col = min(col, self.column_size - 1)
                placeholder_name = inst_name + "{}xbit_r{}_c{}".format(OPTS.hier_seperator, valid_row, valid_col)
                placeholder_inst = self.cell_inst[valid_row, valid_col]
                return placeholder_name, placeholder_inst
            else:
                debug.error("Cell ({}, {}) not found in bitcell array. Array size: {}x{}".format(
                    row, col, self.row_size, self.column_size), 1)
