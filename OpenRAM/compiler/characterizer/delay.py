...
#        # but is never mentioned otherwise
        ...
        # For analytical delay mode, these paths aren't critical - allow missing paths
        if OPTS.analytical_delay:
            if len(sen_paths) != 1:
                debug.warning('Found {0} paths which contain the s_en net (expected 1). Using dummy path for analytical delay.'.format(len(sen_paths)))
                sen_path = [] if len(sen_paths) == 0 else sen_paths[0]
            else:
                sen_path = sen_paths[0]
            if len(bl_paths) != 1:
                debug.warning('Found {0} paths which contain the bitline net (expected 1). Using dummy path for analytical delay.'.format(len(bl_paths)))
                bitline_path = [] if len(bl_paths) == 0 else bl_paths[0]
            else:
    ...
    def check_arguments(self):
        ...
        # With analytical delay and multi-bank configs, address size checking can be relaxed
        if len(self.probe_address) != self.bank_addr_size:
            if OPTS.analytical_delay:
                debug.warning("Probe Address's number of bits ({}) does not match bank_addr_size ({}) - expected in multi-bank configs with analytical delay.".format(
                    len(self.probe_address), self.bank_addr_size))
            else:
