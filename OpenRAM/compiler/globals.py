...
# Get OPENRAM_HOME from environment or calculate it
OPENRAM_HOME = os.environ.get("OPENRAM_HOME", os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + "/compiler")
...
def install_conda():
    ...
    if os.path.exists(OPENRAM_HOME + "/install_conda.sh"):
        ...
        debug.warning("Couldn't find conda installer script.")
        return
...
def setup_paths():
    ...
    # OPENRAM_HOME is already defined at module level
...
def import_tech():
    ...
    try:
        import openram
        if OPENRAM_TECH != "":
            OPENRAM_TECH += os.pathsep
        # Handle namespace packages where __file__ might be None
        if hasattr(openram, '__file__') and openram.__file__:
            tech_dir = os.path.dirname(openram.__file__) + "/technology"
        else:
            # Use __path__ for namespace packages
            tech_dir = os.path.dirname(list(openram.__path__)[0]) + "/technology"
        OPENRAM_TECH += tech_dir
    except:
        if OPENRAM_TECH == "":
    ...
    for tech_path in OPENRAM_TECH.split(os.pathsep):
    ...
    try:
        # Import tech directly using importlib to avoid circular import
        import importlib.util
        tech_init_path = os.path.join(tech_path, "tech", "__init__.py")
        spec = importlib.util.spec_from_file_location("openram.tech", tech_init_path)
        tech = importlib.util.module_from_spec(spec)
        sys.modules["openram.tech"] = tech
        spec.loader.exec_module(tech)
        # Also set it as an attribute of openram for backward compatibility
        openram.tech = tech
    except Exception as e:
        debug.error("Could not load tech module: {}".format(str(e)), -1)
